...
min_temp = round_any(min(spc_ls_wtsd_temps$mean_c), 0.5, f = floor)
max_temp = round_any(max(spc_ls_wtsd_temps$mean_c), 0.5, f = ceiling)
...
wtsd_spc_ls_p = spc_ls_wtsd_temps %>%
  ...
  scale_y_continuous(breaks = seq(min_temp, max_temp, by = 0.25)) +
