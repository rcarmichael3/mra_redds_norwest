...
aug_temp_rkm = mra_norwest %>%
  select(FTYPE, NrWST_r, GNIS_NA, COMID, 
         starts_with('S'),
         -SLOPE) %>%
  ...
  dplyr::select(-FTYPE, -NrWST_r, -COMID, -GNIS_NA, -River) %>%
  dplyr::select(watershed, rkm, yr, everything()) %>%
  arrange(watershed, rkm, scenario_num) %>%
  mutate(watershed = recode(watershed, 
                            "Salmon" = "Upper_Salmon")) %>%
  # some rkm's have more than one value, just pick the first
  group_by(watershed, scenario_num, rkm) %>%
  slice(1) %>%
  ungroup()
...
Figure \@ref(fig:lemh-redds-n-temps) provides a longitudinal temperature profile, in this case the mean of 8-day maximum temperature predictions for August 29, along with mean redd densities averaged across years for which survey data were available. Figure \@ref(fig:lemh-redds-n-nrwt-temps) provides similar information, but uses the NorWeST model's predicted August temperature, averaged across years 2002-2011.
