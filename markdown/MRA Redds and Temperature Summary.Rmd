...
Figure \@ref(fig:lemh-redds-n-temps) provides a longitudinal temperature profile, in this case the mean of 8-day maximum temperature predictions for August 29, along with mean redd densities averaged across years for which survey data were available.

<!-- 
I have an issue I need to resolve with the below plot and the following similar plots for Pahsimeroi and Upper Salmon. I have code within them to add a geom_ribbon for the optimum temps and geom_hlines for the max and acute temps. However, they don't show when I add the limits argument to scale_y_continuous. If I remove that argument, the geom_ribbons and geom_hlines show; however, the secondary y-axis expands to include the full range of the thresholds, which makes the plot look terrible. Need to find a way to better control the limits of the y-axis while also displaying the threshold lines.
-->
...
lemh_temp_rkm_sf %>%
  ...
  geom_ribbon(aes(ymin = 7.2, ymax = 14.5),
              fill = "green2",
              alpha = 0.2) +
  geom_bar(aes(y = mean_redds_km * coeff),
           stat = "identity",
           alpha = 0.3,
           fill = "steelblue4",
           color = "steelblue4") +
...
# sec_y_0 = 12 # set origin of the secondary y-axis
# lemh_temp_rkm_sf %>%
#   left_join(lemh_redd_rkm) %>%
#   ggplot(aes(x = rkm)) +
...
#               size = 1, colour = "red3") +
#   # optimum temp range
#   geom_ribbon(aes(ymin = (7.2 - sec_y_0) * coeff,
#                   ymax = (14.5 - sec_y_0) * coeff),
#               fill = "green2",
#               alpha = 0.2) +
#   # max temp threshold
#   geom_hline(aes(yintercept = (18 - sec_y_0) * coeff),
#              colour = "red", linetype = "longdash") +
#   # acute temp threshold
#   geom_hline(aes(yintercept = (20 - sec_y_0) * coeff),
#              colour = "darkred") +
#   scale_x_continuous(breaks = seq(0, 150, by = 5)) +
...
Figure \@ref(fig:pahs-redds-n-temps) provides a longitudinal temperature profile, in this case the mean of 8-day maximum temperature predictions for August 29, along with mean redd densities averaged across years for which survey data were available.
...
Figure \@ref(fig:upsa-redds-n-temps) provides a longitudinal temperature profile, in this case the mean of 8-day maximum temperature predictions for August 29, along with mean redd densities averaged across years for which survey data were available.
...
* Redd locations (i.e., the core spawning areas) in the Lemhi River do seem to occur where stream temperatures are appropriate or okay. Interesting. I'm wondering, does historic redd distribution information suggest redds lower in system if and when stream temps were cooler prior to so much irrigation during summer months? But it does seem that temperatures are okay in the current core spawning area.
* On the contrary, redd distributions in the Pahsimeroi and Upper Salmon occur in areas where temps seem way too high. Couple things...redds are largely constrained to occur below the weirs. Those redds, too, are likely dominated by hatchery fish. Has the presence of a weir hindered the ability or encouraged wild fish to spawn low in the systems where stream temps are too high? And for those few wild redds that do occur below the weirs, they are probably dominated or overrun by hatchery redds. I wouldn't doubt if there's imposition of redds in those limited locations. In Pahsimeroi, I don't have a great sense for how much appropriate spawning areas there are above the weir, but at least some of the habitat Richie and I walked, particularly in Patterson Creek, seemed like decent spawning habitat that, at least according to this data, doesn't seem to be getting used.
* Temperatures during the extreme seasons (mid-summer and mid-winter) do seem to be limiting. High summer temps likely cause stress for holding and spawning adults, potentially causing pre-spawn mortality or reduced fitness during spawning. And high temps likely increase 'metabolic stress' and food requirements for parr, also perhaps increasing stress and decreasing body condition prior to fall and spring. Does that encourage the presmolt life history? Further temperatures during the winter are too low for winter rearing, which isn't surprising, but can temperatures be mediated during these extreme seasons. It seems that hyporheic exchange may be key to moderating temps during the extreme seasons! More evidence to increase complexity, channel unit frequency, etc.
