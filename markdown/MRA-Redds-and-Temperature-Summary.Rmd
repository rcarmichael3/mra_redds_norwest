...
temp_rkm_sf = temp_rkm_sf %>%
  ...
  filter(!(watershed == 'Pahsimeroi' & River == "Salmon River")) %>%
  group_by(watershed, year, variable, 
           RCAID,
           River, rkm) %>%
...
```{r}
# put temperature into a data.frame for plotting purposes
plot_df = temp_rkm_sf %>%
  st_drop_geometry() %>%
  ...
  mutate(across(jday,
         ...
         across(jday,
                as.numeric)) %>%
  group_by(watershed, year, variable, rkm, jday) %>%
  ...
  crossing(thresh_df %>%
             filter(species == 'chinook',
                    life_stage %in% c("summer_parr",
                                      "spawning",
                                      "winter_presmolt")) %>%
             filter(life_stage != "winter_presmolt") %>%
             select(-ends_with("date")) %>%
             pivot_wider(names_from = "threshold",
                         values_from = "temp_c")) %>%
  rowwise() %>%
  mutate(season = if_else(life_stage != "winter_presmolt",
                          if_else(between(jday, start_julian, end_julian), T, F),
                          if_else(jday >= start_julian | jday <= end_julian, T, F)),
         opt_temp = if_else(between(temp, optimum_low, optimum_high), T, F),
         tol_temp = if_else(between(temp, minimum, maximum), T, F),
         bel_acute = if_else(between(temp, minimum, acute), T, F)) %>%
  ungroup() %>%
  arrange(species, 
          life_stage,
          watershed, 
...
temp_list_p = plot_df %>%
  filter(!(watershed == "Pahsimeroi" & rkm > 45)) %>%
  # filter(watershed == "Lemhi") %>%
  # split(list(.$life_stage)) %>%
  split(list(.$watershed)) %>%
  map(.f = function(y) {
    y %>% 
      split(list(.$life_stage)) %>%
      ...
      map(.f = function(x) {
        lfstg = x %>%
          select(life_stage) %>%
          distinct() %>%
          mutate(life_stage = recode(life_stage,
                                     "spawning" = "Spawning (8/10 - 9/30)",
                                     "summer_parr" = "Summer Parr (6/10 - 10/15)",
                                     "winter_presmolt" = "Winter Presmolt (11/14 - 3/01)")) %>%
          pull(life_stage)
        
        if(grepl("Winter", lfstg)) {
          x = x %>%
            mutate(jday = if_else(jday > 200, jday - 365, jday))
        }
        
        x %>%
          mutate(temp = if_else(opt_temp, temp, NA_real_)) %>%
          filter(season) %>%
          mutate(date = ymd("20190101") + days(jday - 1)) %>%
          ggplot(aes(x = date,
                     y = rkm,
                     fill = temp)) +
          geom_tile() +
...
for(wtsd in names(wtsd_p_list)) {
  ggsave(paste0("../figures/", wtsd, "_temp_rkm_fig.pdf"),
         plot = wtsd_p_list[[wtsd]],
         width = 6.5,
         height = 6,
         dpi = 300)
}
...
lemh_temp_rkm_sf = avg_temp_rkm_sf %>%
  filter(watershed == 'Lemhi') %>%
  filter(rkm > 1) # an odd model estimate at confluence with Salmon R.
...
```{r lemh-redds-n-temps, fig.cap = "Mean redd density (redds/km) by river kilometer in the Lemhi River averaged across years for which surveys were completed and data available. River kilometer 0 is at the confluence with the Salmon River. River kilometer 93 is the upstream extent of the mainstem Lemhi River and occurs at the Highway 29 bridge near Leadore and the confluence of Eighteenmile and Texas Creeks. A longitudinal temperature profile, with smoothed line (red), showing predictions of the mean of 8-day maximum temperatures at August 29 (averaged across available years) is also shown. The green shaded area are the optimum temperatures for Chinook spawning. Numbers at the top and dashed vertical lines designate geomorphic reaches."}
...
lemh_redd_rkm = mra_redds %>%
  ...
  st_join(mra_rkm,
          join = st_nearest_feature,
          left = TRUE) %>%
  group_by(rkm, GeoReach, SurveyYear) %>%
  summarise(n_redds = n()) %>%
  ungroup() %>%
    complete(mra_rkm %>%
             filter(River == 'Lemhi River') %>%
             as_tibble() %>%
             dplyr::select(rkm, GeoReach) %>%
             mutate(GeoReach = fct_drop(GeoReach)),
           SurveyYear,
           fill = list(n_redds = 0)) %>% # need to set max rkm here
  dplyr::select(- geometry) %>%
  group_by(rkm, GeoReach) %>%
  summarise(mean_redds_km = mean(n_redds)) %>%
  ungroup()

# for plotting geomorphic reach breaks
lemh_geo_rch_label = lemh_redd_rkm %>%
  group_by(GeoReach) %>%
  summarise(min_rkm = min(rkm),
            max_rkm = max(rkm)) %>%
...
lemh_temp_rkm_sf %>%
  ...
  geom_vline(data = lemh_geo_rch_label,
             aes(xintercept = sep_rkm),
             linetype = 2,
             color = 'gray80') +
  geom_text(data = lemh_geo_rch_label,
            aes(label = label_txt,
                x = label_rkm),
            y = 16.3,
            size = 3) +
...
```{r lemh-redds-n-nrwt-temps, fig.cap = "Mean redd density (redds/km) by river kilometer in the Lemhi River averaged across years for which surveys were completed and data available. River kilometer 0 is at the confluence with the Salmon River. River kilometer 93 is the upstream extent of the mainstem Lemhi River and occurs at the Highway 29 bridge near Leadore and the confluence of Eighteenmile and Texas Creeks. A longitudinal temperature profile, with smoothed line (red), showing predictions of the average August temperature from the NorWeST model (averaged across the years 2002-2011) is also shown. The green shaded area are the optimum temperatures for Chinook spawning. Numbers at the top and dashed vertical lines designate geomorphic reaches."}
...
```{r lemh-chnk-temp_thres, fig.cap = "Heat map showing the 8-day mean of modelled maximum temperatures by date and river kilometer in the Lemhi, faceted by year. Dark red indicates the temperature was outside the optimal range for that Chinok lifestage."}
wtsd_p_list$Lemhi
```
...
pahs_temp_rkm_sf = avg_temp_rkm_sf %>%
  ...
  filter(rkm > 0) # observation with an NA at confluence with Salmon R.
...
```{r pahs-redds-n-temps, fig.cap = "Mean redd density (redds/km) by river kilometer in the Pahsimeroi River averaged across years for which surveys were completed and data available. River kilometer 0, the downstream extent, is at the confluence with the Salmon River. River kilometer 86 is the upstream extent of the mainstem Pahsimeroi River and occurs at the confluence of the West Fork and East Fork Pahsimeroi River. A longitudinal temperature profile, with smoothed line (red), showing predictions of the mean of 8-day maximum temperatures at August 29 (averaged across available years) is also shown. The green shaded area are the optimum temperatures for Chinook spawning. Numbers at the top and dashed vertical lines designate geomorphic reaches."}
# calculate average redds per rkm
pahs_redd_rkm = mra_redds %>%
  ...
  mutate(GeoReach = factor(GeoReach,
                           levels = unique(GeoReach)))

# for plotting geomorphic reach breaks
pahs_geo_rch_label = pahs_redd_rkm %>%
...
pahs_temp_rkm_sf %>%
  ...
  geom_ribbon(aes(ymin = 7.2, 
              ...
              alpha = 0.2) +
  # mark out geomorphic reaches
  geom_vline(xintercept = c(min(pahs_geo_rch_label$min_rkm), 
                            pahs_geo_rch_label$sep_rkm),
             linetype = 2,
             color = 'gray80') +
  geom_text(data = pahs_geo_rch_label,
            aes(label = label_txt,
                x = label_rkm),
            y = 16.3,
            size = 3) +
...
```{r pahs-redds-n-nrwt-temps, fig.cap = "Mean redd density (redds/km) by river kilometer in the Pahsimeroi River averaged across years for which surveys were completed and data available. River kilometer 0, the downstream extent, is at the confluence with the Salmon River. River kilometer 86 is the upstream extent of the mainstem Pahsimeroi River and occurs at the confluence of the West Fork and East Fork Pahsimeroi River. A longitudinal temperature profile, with smoothed line (red), showing predictions of the average August temperature from the NorWeST model (averaged across the years 2002-2011) is also shown. The green shaded area are the optimum temperatures for Chinook spawning. Numbers at the top and dashed vertical lines designate geomorphic reaches."}
...
```{r pahs-chnk-temp_thres, fig.cap = "Heat map showing the 8-day mean of modelled maximum temperatures by date and river kilometer in the Pahsimeroi, faceted by year. Dark red indicates the temperature was outside the optimal range for that Chinok lifestage."}
wtsd_p_list$Pahsimeroi
```
...
```{r upsa-redds-n-temps, fig.cap = "Mean redd density (redds/km) by river kilometer in the Upper Salmon River (above Redfish Lake Creek) averaged across years for which surveys were completed and data available. River kilometer 0, the downstream extent, is at the confluence with Redfish Lake Creek. River kilometer 56 is the upstream extent of the mainstem Upper Salmon River at its source. A longitudinal temperature profile, with smoothed line (red), showing predictions of the mean of 8-day maximum temperatures at August 29 (averaged across available years) is also shown. The green shaded area are the optimum temperatures for Chinook spawning. Numbers at the top and dashed vertical lines designate geomorphic reaches."}
# calculate average redds per rkm
...
# for plotting geomorphic reach breaks
upsa_geo_rch_label = upsa_redd_rkm %>%
  filter(!is.na(GeoReach)) %>%
  group_by(GeoReach) %>%
  summarise(min_rkm = min(rkm),
            max_rkm = max(rkm)) %>%
...
upsa_temp_rkm_sf %>%
  ...
  geom_ribbon(aes(ymin = 7.2, 
              ...
              alpha = 0.2) +
  # mark out geomorphic reaches
  geom_vline(xintercept = c(min(upsa_geo_rch_label$min_rkm),
                            upsa_geo_rch_label$sep_rkm),
             linetype = 2,
             color = 'gray80') +
  geom_text(data = upsa_geo_rch_label,
            aes(label = label_txt,
                x = label_rkm),
            y = 16.3,
            size = 3) +
...
```{r upsa-redds-n-nrwt-temps, fig.cap = "Mean redd density (redds/km) by river kilometer in the Upper Salmon River (above Redfish Lake Creek) averaged across years for which surveys were completed and data available. River kilometer 0, the downstream extent, is at the confluence with Redfish Lake Creek. River kilometer 56 is the upstream extent of the mainstem Upper Salmon River at its source. A longitudinal temperature profile, with smoothed line (red), showing predictions of the average August temperature from the NorWeST model (averaged across the years 2002-2011) is also shown. The green shaded area are the optimum temperatures for Chinook spawning. Numbers at the top and dashed vertical lines designate geomorphic reaches."}
...
```{r upsa-chnk-temp_thres, fig.cap = "Heat map showing the 8-day mean of modelled maximum temperatures by date and river kilometer in the Upper Salmon, faceted by year. Dark red indicates the temperature was outside the optimal range for that Chinok lifestage."}
wtsd_p_list$Pahsimeroi
```
